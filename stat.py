...
class Switch:
  ...
  def income_pkt (self, pkt):
    ...
    try:
      ...
      rule.increment_uses()
      if (pkt[SwitchTrace].qdepth > QUEUE_THRESHOLD or pkt[SwitchTrace].timedelta) and time.time() - self.last_congestion_print > CONGESTION_TIME:
        print_congestion()
  ...
  def print_congestion(self):
    ...
    active_flows = [f for f in flows.values() if f.active]
    for f in active_flows:
      print '\tFlow from ' + str(f.src) + ' started ' + str(time.time() - f.init_time) + ' seconds ago -> ' + str(f.num_of_pkts)
    print 'This congestion is causing an average delay of ' + str(self.avg_delay) + 'ms'
    print 'The queue ocupacy of this switch is ' + str(self.avg_queue_ocupacy) + '%'
    print 'The forwarding rules of this switch are:'
    for rule in self.rules.values():
      print '\t' + str(rule.id) + ') ' + str(rule.key_addr) + '/' + str(rule.prefix_size) + ' => port ' + str(rule.port) + ' (used ' + str(rule.times_used) + ' times)'
...
def handle_pkt(pkt):
    ...
    print 'timestamp = ' + str(pkt[SwitchTrace].timestamp)
